-- Script para corrigir erro de views existentes
-- Data: 2024
-- Descrição: Remove e recria views para evitar conflitos de estrutura
...
-- 3. Remover views existentes se existirem
DROP VIEW IF EXISTS class_tracking_report CASCADE;
DROP VIEW IF EXISTS students_per_class_report CASCADE;

-- 4. Verificar se a função get_dropout_students existe e está funcionando
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 5. Criar views para substituir o uso do método .modify() no Supabase
-- View para class_tracking_report
CREATE VIEW class_tracking_report AS
SELECT 
    cl.id as class_id,
    cl.name as class_name,
    c.id as course_id,
...
CREATE VIEW students_per_class_report AS
SELECT 
...
-- Mensagem de sucesso
SELECT 'Script executado com sucesso! Views recriadas e coluna last_sign_in_at adicionada.' as resultado;