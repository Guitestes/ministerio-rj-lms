...
const generateCertificate = async (courseId: string, userId: string): Promise<Certificate> => {
  try {
    ...
    if (isEligible) {
      // 2a. Validar frequência mínima
      console.log(`[CERTIFICADO] Validando frequência para usuário ${userId} no curso ${courseId}`);
      const MINIMUM_ATTENDANCE_RATE = 75.0;

      try {
        // Encontrar a turma (class_id) do aluno para este curso
        const { data: classEnrollment, error: classEnrollError } = await supabase
            .from('enrollments')
            .select('class_id, classes (id, course_id)')
            .eq('user_id', userId)
            .single();
        ...
        if (classEnrollError || !classEnrollment || (classEnrollment.classes as { course_id: string }[])?.[0]?.course_id !== courseId) {
          ...
          // Here we should call the function to get the summary
          const { data: attendanceSummary, error: attendanceError } = await supabase.rpc('get_student_attendance_summary', {
            class_id_param: classId,
          });

          if (attendanceError) {
            console.warn('[CERTIFICADO] Erro ao buscar resumo de frequência. Ignorando a verificação de frequência.', attendanceError);
          } else if (attendanceSummary && attendanceSummary.length > 0) {
            const userAttendance = attendanceSummary.find(s => s.user_id === userId);
            if (userAttendance) {
              console.log(`[CERTIFICADO] Frequência do usuário: ${userAttendance.attendance_rate}%. Mínimo: ${MINIMUM_ATTENDANCE_RATE}%`);
              if (userAttendance.attendance_rate < MINIMUM_ATTENDANCE_RATE) {
                throw new Error(`Não elegível para certificado. Frequência de ${parseFloat(userAttendance.attendance_rate).toFixed(2)}% é inferior ao mínimo de ${MINIMUM_ATTENDANCE_RATE}%.`);
              }
            } else {
               console.warn(`[CERTIFICADO] Resumo de frequência encontrado para a turma, mas não para o usuário ${userId}. Ignorando.`);
            }
          }
        }
      } catch (attendanceValidationError) {
        console.error('[CERTIFICADO] Erro na validação de frequência:', attendanceValidationError);
        // Re-throw the specific error to be caught by the main try-catch block
